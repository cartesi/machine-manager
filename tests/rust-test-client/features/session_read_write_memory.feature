...
Feature: SessionReadWriteMemory feature
    ...
    Scenario Outline: read from pristine machine
        ...
        Examples:
            ...
            |   1   |   0    |   36028797018963970 |   16   | 00000000000000000000000000000000 |
            |  30   |   0    |   36028797018963970 |   16   | 00000000000000000000000000000000 |
    ...
    Scenario Outline: read written value
        ...
        Examples:
            ...
            |  30   |   0    |  36028797018963970 |   16   | HELLOWORLD | 48454C4C4F574F524C44000000000000 |
    ...
    Scenario Outline: read on invalid cycle
        ...
        Given machine manager server is up
        And a machine manager server with a machine executed for 20 final cycles and 0 final ucycles
        When client asks server to read memory on cycle <cycle> and ucycle <ucycle>, starting on address <address> for length <length>
        Then machine manager server returns an InvalidArgument error

        Examples:
            | cycle | ucycle |       address      | length |
            |   5   |   0    | 36028797018963970  |   16   |
    ...
    Scenario Outline: write on invalid cycle
        ...
        Given machine manager server is up
        And a machine manager server with a machine executed for 20 final cycles and 0 final ucycles
        When client asks server to write data <data> on cycle <cycle> and ucycle <ucycle>, starting on address <address>
        Then machine manager server returns an InvalidArgument error

        Examples:
            | cycle | ucycle |       address      |    data    |
            |   5   |   0    |  36028797018963970 | HELLOWORLD |
